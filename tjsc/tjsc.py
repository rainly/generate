...
#headflag => 1:No. 20180424-033, 2:No. 20180422 3:#No. 1766125
#buyflag  => 1:单面，2：双面
config_cptype = {}
config_cptype["天津时时彩"] = {"BallNum":5, "headflag":1, "buyflag":1, "bubit":2}
...
def httpreg(softname, version, phydriverserial, regkey):
    ...
    str = src + signkey
    str = str.encode("utf8")
    #phydriverserial=123&regkey=456&softname=tjsc&version=1.0.0.0&key=0z#z#b#094kls#040jkas892#z#z#b#0
    #phydriverserial=123&regkey=456&softname=tjsc&version=1.0.0.0&key=0z#z#b#094kls#040jkas892#z#z#b#0
    m.update( str )
    result = m.hexdigest()
    ...
    def target_func(self):
        ...
        while self.stopped == False:
            try:
                ...
                Cur_Award_Issue2_Road = [GetType(Ball01), GetType(Ball02), GetType(Ball03), GetType(Ball04), GetType(Ball05)]
...
class Application(tk.Tk):
    ...
    def createTab1(self):
        ...
        self.cptypeChosen['values'] = (1, 2, 4, 42, 100)     # 设置下拉列表的值
    ...
    def saveMe(self):
        ...
        #写回配置文件
        self.conf.write(open("tjsc.txt","w"))
        if self.Paser() == False:
            messagebox.showinfo("提示","方案配置有问题！")
            return  
...
def main():
    app = Application()
    app.title("BB彩票-天津时时彩(双面玩法)(开发者QQ：87954657)")
