...
class MianWindow(basewin.BaseMainWind):  
    ...
    def init_main_window(self):  
        ...
        if self.conf.has_section("app_id") == True:
            ...
            self.conf.add_section("app_id")
            self.conf.set("app_id", "value", "1") 
            
        if self.conf.has_section("app_private_key") == True:
            alipay_client_config.app_private_key = self.conf.get("app_private_key", "value")
        else:
            self.conf.add_section("app_private_key")
            self.conf.set("app_private_key", "value", "1") 
        ...
        if self.conf.has_section("alipay_public_key") == True:
            ...
            self.conf.add_section("alipay_public_key")
            self.conf.set("alipay_public_key", "value", "1") 
            
        if self.conf.has_section("prefix") == True:
            self.m_prefix.SetValue(self.conf.get("prefix", "value"))
        else:
            self.conf.add_section("prefix")
            self.conf.set("prefix", "value", "1") 
        ...
        if self.conf.has_section("username") == True:
            ...
            self.conf.set("username", "value", "1") 
            
        if self.conf.has_section("password") == True:
            self.m_userpwd.SetValue(self.conf.get("password", "value"))
        else:
            self.conf.add_section("password")
            self.conf.set("password", "value", "1") 
    ...
    def onStart(self, event):
        ...
        prefix = self.m_prefix.GetValue()
        prefix = prefix.replace("/", "\n")
        mobiles = prefix.split("\n")

        for mobile in mobiles:
            left_max = 0
            left_len = 11 - len(mobile)
            for i in range(left_len):
                left_max = left_max * 10 + 9    
            
            for i in range(left_max):
                s      = str(i)
                item = mobile + s.zfill(left_len)
                g_mobiles.append(item)
