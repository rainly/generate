...
#def main():
#    app = Application()
#    app.title("Python 图形用户界面(开发者QQ：87954657)")
#    # 主消息循环:
#    app.mainloop()
...
#用config对象读取配置文件
conf.read("test.cfg")
PostUrl="file:///D:/yl1666/Documents/GitHub/test/test.html"
#PostUrl="http://bw1.cpa700.com/"

if conf.has_section("current_url"):
    PostUrl = conf.get("current_url", "value")
...
for section in sections:
    if conf.has_section(section) == False:
        continue;
    str_val = conf.get(section, "Value")
    driver.add_cookie({'name':section, 'value':str_val})

#遍历cookies中的name 和value信息打印，当然还有上面添加的信息
for cookie in driver.get_cookies():
    print (cookie['name'] + ":" + cookie['value'])
    #检测section是否存在
    if conf.has_section(cookie['name']) == False:
        #增加新的section
        conf.add_section(cookie['name'])
    #更新指定section，option的值
    conf.set(cookie['name'], "Value", cookie['value'])
#写回配置文件
...
while True:
    try:
        ...
        for cookie in driver.get_cookies():
            ...
            conf.set(cookie['name'], "Value", cookie['value'])
        #检测section是否存在
        if conf.has_section("current_url") == False:
            #增加新的section
            conf.add_section("current_url")
        #更新指定section，option的值
        conf.set("current_url", "Value", driver.current_url)
        #写回配置文件
        conf.write(open("test.cfg", "w"))
        ##############################
        ##############################
