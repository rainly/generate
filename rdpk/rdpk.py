...
def httpreg(softname, version, phydriverserial, regkey):
    ...
    str = src + signkey
    str = str.encode("utf8")
    #phydriverserial=123&regkey=456&softname=rdpk&version=1.0.0.0&key=0z#z#b#094kls#040jkas892#z#z#b#0
    #phydriverserial=123&regkey=456&softname=rdpk&version=1.0.0.0&key=0z#z#b#094kls#040jkas892#z#z#b#0
    m.update( str )
    result = m.hexdigest()
    ...
    def target_func(self):
        ...
        while self.stopped == False:
            try:
                ...
                Cur_Award_Issue2_Road = [GetType(Ball01), GetType(Ball02), GetType(Ball03), GetType(Ball04), GetType(Ball05), GetType(Ball06), GetType(Ball07), GetType(Ball08), GetType(Ball09), GetType(Ball10)]
...
class Application(tk.Tk):
    ...
    def saveMe(self):
        ...
        #写回配置文件
        self.conf.write(open("rdpk.txt","w"))
        if self.Paser() == False:
            messagebox.showinfo("提示","方案配置有问题！")
            return  
...
def main():
    app = Application()
    app.title("BB彩票-雷电PK(双面玩法)(开发者QQ：87954657)")
