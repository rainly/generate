...
url = "http://77t.snk686.com/search.aspx"
#wd:64801
dict = {'wd':64801}
...
class TestThread(threading.Thread):
    ...
    def target_func(self):
        #http://31nudgw.ttx158.com/cp5-5-ag/checknum.aspx?ts=1521814645944
        print(loginurl)
        print(logindict)
        pass;
...
class Application(tk.Tk):
    ...
    def createTab1(self):
        ...
        #行
        line = line + 1
        self.label = tk.Label(self.MyFrame, image=self.tk_image, bg='brown')
    ...
    def clickMe(self):
        ...
        print(loginurl)
        print(logindict)
        logindict["txt_U_name"]     = self.name
        logindict["txt_U_Password"] = self.pwd 
        logindict["txt_validate"]   = self.check
        
        data = urllib.parse.urlencode(logindict).encode('utf-8')
        request = urllib.request.Request(url = loginurl, data = data, headers = headers, method = 'POST')
        try:
            response = opener.open(request, timeout = 5)
            html = response.read().decode()
        except urllib.error.HTTPError as e:
        ...
        url = baseurl + "ch/agreement.aspx/LocationUrl"
        print(url)
        request = urllib.request.Request(url = url, data = data, headers = headers, method = 'POST')
        try:
            response = opener.open(request, timeout = 5)
        ...
        agreement_dict["stype"] = "1"
        data = urllib.parse.urlencode(agreement_dict).encode('utf-8')
        url = baseurl + "opadmin/mreport_new.aspx"
        print(url)
        request = urllib.request.Request(url = url, data = data, headers = headers, method = 'POST')
        try:
            response = opener.open(request, timeout = 5)
        ...
        while True:
            ...
            url = baseurl + "opadmin/mreport_new_detail.aspx?memberno=tbgd002&gameno=6;8;11;12;13;20;21;22;23;&sdate=2018-03-24&edate=2018-03-24&roundno1=&roundno2=&wagerroundno=&wagertypeno=&onlyself=0&isbupai=&isjs=0&datetime=2018-03-24&curpage=1&ts=1521858951523"
            print(url)
